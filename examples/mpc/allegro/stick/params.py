...
class ExplicitMPCParams:
    def __init__(self, rand_seed=1, target_type='rotation'):
        ...
        # ---------------------------------------------------------------------------------------------
        self.model_path_ = 'envs/xmls/env_allegro_stick.xml'
        ...
        if target_type == 'rotation':
            init_obj_xy = np.array([-0.01, 0.0]) + 0.005 * np.random.randn(2)
            init_obj_pos = np.hstack([init_obj_xy, 0.03])
            # init_yaw_angle = np.pi * np.random.rand(1) - np.pi / 2
            init_yaw_angle = np.pi/2 * np.random.rand(1) + np.pi / 4
            ...
            self.target_p_ = np.array([-0.01, -0.0, 0.03])
